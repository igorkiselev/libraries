...
// imagesloaded
if (get_option('libraries-imagesloaded')) {
...
// Google map
if (get_option('libraries-google-map')) {
    add_action('wp_enqueue_scripts', function () {

    $link = 'http://maps.googleapis.com/maps/api/js?v=3.exp&language=ru&libraries=places';

    if (get_option('libraries-google-map-key')) {
        $link = $link.'&key='.get_option('libraries-google-map-key');
    }

    if (get_option('libraries-google-map-limit')) {
        $array = explode(',', get_option('libraries-google-map-limit'));
...
if (get_option('libraries-google-map-key') && class_exists('acf')) {
    add_action('acf/init', function () {

        acf_update_setting('google_api_key', get_option('libraries-google-map-key'));

    });

    add_filter('acf/fields/google_map/api', function ($api) {

        $api['key'] = get_option('libraries-google-map-key');
...
if (get_option('libraries-justbenice-editor')) {
...
// Google  Analytics
if (get_option('libraries-google-analytics') && get_option('libraries-google-analytics-key')) {
    add_action('wp_footer', function () {
            echo "<script>(function(b,o,i,l,e,r){b.GoogleAnalyticsObject=l;b[l]||(b[l]=function(){(b[l].q=b[l].q||[]).push(arguments)});b[l].l=+new Date;e=o.createElement(i);r=o.getElementsByTagName(i)[0];e.src='//www.google-analytics.com/analytics.js';r.parentNode.insertBefore(e,r)}(window,document,'script','ga'));ga('create','".get_option('libraries-google-analytics-key')."');ga('send','pageview');</script>\n";

    });
}
// Yandex Metrics
if (get_option('libraries-yandex-metrics') && get_option('libraries-yandex-metrics-key')) {
    add_action('wp_footer', function () {
            echo '<!-- Yandex.Metrika counter --> <script type="text/javascript" > (function (d, w, c) { (w[c] = w[c] || []).push(function() { try { w.yaCounter'.get_option('libraries-yandex-metrics-key').' = new Ya.Metrika({ id:'.get_option('libraries-yandex-metrics-key').', clickmap:true, trackLinks:true, accurateTrackBounce:true, webvisor:true, trackHash:true }); } catch(e) { } }); var n = d.getElementsByTagName("script")[0], s = d.createElement("script"), f = function () { n.parentNode.insertBefore(s, n); }; s.type = "text/javascript"; s.async = true; s.src = "https://mc.yandex.ru/metrika/watch.js"; if (w.opera == "[object Opera]") { d.addEventListener("DOMContentLoaded", f, false); } else { f(); } })(document, window, "yandex_metrika_callbacks"); </script> <noscript><div><img src="https://mc.yandex.ru/watch/45776769" style="position:absolute; left:-9999px;" alt="" /></div></noscript> <!-- /Yandex.Metrika counter -->';
    });
}

if (get_option('libraries-filenames') && get_option('libraries-filenames-slug')) {
    add_filter('sanitize_file_name', function ($filename) {
        $slug = get_option('libraries-filenames-slug');
...
if (get_option('libraries-lazy-srcset')) {
...
if (get_option('libraries-lazy-brakepoints')) {
    if (get_option('libraries-lazy-brakepoints-sizes')) {
        add_action('after_setup_theme', function () {
            $array = explode(',', get_option('libraries-lazy-brakepoints-sizes'));
            if (!empty($array)) {
...
if (get_option('libraries-owlcarousel-gallery')) {
...
if (get_option('libraries-opengraph')) {
...
if (get_option('libraries-disable-emoji')) {
...
if (get_option('libraries-disable-adminbar')) {
    add_filter('show_admin_bar', '__return_false');
}
if (get_option('libraries-disable-generator')) {
    remove_action('wp_head', 'wp_generator');
}
if (get_option('libraries-disable-rsslinks')) {
    remove_action('wp_head', 'feed_links_extra', 3);
    remove_action('wp_head', 'feed_links', 2);
    remove_action('wp_head', 'rsd_link');
    remove_action('wp_head', 'wlwmanifest_link');
}
if (get_option('libraries-disable-rellinks')) {
...
if (get_option('libraries-header-wp_title')) {
    add_filter('wp_title', function ($title) {
        return $title.esc_attr(get_bloginfo('name'));
    });
}
if (get_option('libraries-featured-rss')) {
...
if (get_option('libraries-functions-html5')) {
    add_action('after_setup_theme', function () {
        add_theme_support('html5', array('comment-list', 'comment-form', 'search-form', 'gallery', 'caption'));
    });
}
if (get_option('libraries-functions-post-thumbnails')) {
    add_action('after_setup_theme', function () {
        add_theme_support('post-thumbnails');
    });
}
if (get_option('libraries-functions-ischild')) {
...
if (get_option('libraries-functions-bodyclass')) {
...
if (get_option('libraries-functions-nav-description')) {
...
if (get_option('libraries-functions-escapekey')) {
    add_action('wp_footer', function () {
        global $post;
...
if (get_option('libraries-editor-css-normalize')) {
    add_action('after_setup_theme', function () {
